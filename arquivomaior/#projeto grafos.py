...
def visualizar_subgrafo(grafo, tamanho=100):
    """
    Visualiza uma amostra do grafo para análise visual, com espessura das arestas proporcional ao peso.
    """
    ...
    try:
        ...
        # Normalizar os pesos para uma faixa razoável de espessuras de linha
        min_weight = min(weights.values()) if weights else 1
        max_weight = max(weights.values()) if weights else 1
        if max_weight > min_weight:
            edgewidth = [3 * (weights[(u, v)] - min_weight) / (max_weight - min_weight) + 0.5 for u, v in sub.edges()]
        else:
            edgewidth = [1.0] * sub.number_of_edges() # Espessura constante se todos os pesos forem iguais

        plt.figure(figsize=(12, 8))
        nx.draw(sub, pos, node_size=30, arrows=True, with_labels=False,
                node_color='skyblue', edge_color='gray', width=edgewidth, alpha=0.8)
        plt.title(f"Visualização de Subgrafo ({num_nos_plotar} nós da amostra) - Espessura das Arestas por Frequência")
        plt.savefig('subgrafo_amostra_com_pesos.png', dpi=300, bbox_inches='tight')
        plt.show()
    except Exception as e: print(f"Erro ao visualizar subgrafo com pesos: {e}")
...
def main():
    ...
    caminho_arquivo = r"C:\Users\gonca\OneDrive\Documentos\GitHub\projeto-grafos\arquivomaior\copia"
